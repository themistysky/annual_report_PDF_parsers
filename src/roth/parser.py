...
def format_market_value(text):
    """Post processing for parsed market values."""
    return (
        "-" + re.sub(r"[)(, ]", "", text)
        if re.search(r"[)(]", text)
...
def format_net_assets(text):
    """Post processing for parsed net_assets."""
    return (
...
def clean1(table, report, website, pdf_url, isin):
    ...
    if "visionfund" in website.lower():
        ...
        table["net_assets"] = table["net_assets"].apply(format_net_assets)
    else:
        table["market_value"] = table["market_value"].apply(
            format_market_value
        )
...
def clean2(table, report, website, pdf_url, isin):
    ...
    table = table.reset_index(drop=True)
    table["market_value"] = table["market_value"].apply(format_market_value)
    table["net_assets"] = table["net_assets"].apply(format_net_assets)
    mode = table.currency.mode()[0]
    for i, r in table.iterrows():
        if str(r["currency"]) == "nan":
